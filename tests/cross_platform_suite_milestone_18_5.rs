use dcc_core::{Computation, Digest, OutputManifestItem, PathUtils};
...
fn test_cross_platform_path_normalization_and_sharding() {
    ...
    let unix_style = PathUtils::normalize_relative_path("src/nested/module/file.rs").unwrap();
    let win_style = PathUtils::normalize_relative_path("src\\nested\\module\\file.rs").unwrap();
    let mixed_style = PathUtils::normalize_relative_path("src/nested\\module/file.rs").unwrap();
...
fn test_cross_platform_process_execution_and_environment() {
    ...
    env.insert("DCC_TEST_VAR".to_string(), "ENVIRONMENT_PASSED_OK".to_string());
...
fn test_cross_platform_file_semantics_and_restoration() {
    ...
    let manifest = vec![OutputManifestItem {
    ...
    OutputRestorer::restore_outputs(&env.storage, &manifest, env.workspace_dir.path()).unwrap();
